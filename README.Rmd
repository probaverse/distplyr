...
[![Codecov test coverage](https://codecov.io/gh/probaverse/distplyr/graph/badge.svg?token=O7DN2RBUQ9)](https://codecov.io/gh/probaverse/distplyr)
<!-- badges: end -->
...
`distplyr` is not on CRAN yet. You can download the development version from GitHub with:
...
The creation of `distplyr` would not have been possible without the support of BGC Engineering Inc., Politecnico di Milano, the European Space Agency, The University of British Columbia, and the Natural Science and Engineering Research Council of Canada (NSERC). The authors would also like to thank the reviewers from ROpenSci for their insightful feedback in the `distionary` package, which also helped in the development of `distplyr` by extension.
